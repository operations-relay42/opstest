...
node("ec2-slave") {
  ...
  stage("tf plan") {
    task_desired_count = sh(returnStdout: true, script: 'aws ecs describe-services --cluster hello-app-abc --services hello-app | grep desiredCount | grep -Eo \'[0-9]+\' | head -n1').toInteger()
